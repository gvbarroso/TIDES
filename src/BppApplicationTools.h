/*
  This file is a copy of ApplicationTools.h in the bpp-core libraries available in
  (https://github.com/BioPP/bpp-core/blob/master/src/Bpp/App/ApplicationTools.h)
...
#include "BppFileTools.h"
#include "BppOutputStream.h"
#include "BppTextTools.h"
#include "BppStringTokenizer.h"
#include "BppNestedStringTokenizer.h"
 ...
 */
  ...
  public:
    ...
    template<class T> static std::vector<T> getVectorParameter(
      ...
      int warn = 0)
    {
      if (separator == ' ') throw Exception("BppApplicationTools::getVectorParameter(). Separator cannot be a space character.");
    ...
    template<class T> static std::vector< std::vector<T> > getVectorOfVectorsParameter(
      ...
      int warn = 0)
    {
      if (separator == ' ') throw Exception("BppApplicationTools::getVectorOfVectorsParameter(). Separator cannot be a space character.");
      std::string s = getStringParameter(parameterName, params, defaultValue, suffix, suffixIsOptional, warn);
     ...
     * {
     *   BppApplicationTools::displayGauge(i, 999, '*');
     ...
     * for(size_t i = 0; i < 1000; i++)
     * {
     *   BppApplicationTools::displayUnlimitedGauge(i);
     *   //Perform time consuming task...
     * }
     * @endcode
     * will result in something like:
